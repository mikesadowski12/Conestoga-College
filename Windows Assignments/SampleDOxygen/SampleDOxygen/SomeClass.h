...
/* NOTE : The DOxygen formatting used in this class definition falls inline with C# style commenting that you will be 
	      introduced to next semester ... so this style is okay as well ... */
...
/// - any and all <b>methods</b> in the proper <i>Method Header Commenting</i> style (see SomeClass() below)
...
public:
	// --------------  CONSTANTS --------------
	static const int	SOME_CLASS_STATUS_OK	= 1;	///< These <i>const class data members</i> are used to enumerate some status codes from SomeClass
	...
	// ---------------- METHODS ---------------
	///
	/// \brief <b>Brief Description</b> - To instantiate a new SomeClass object - given a set of attribute values
	/// \details <b>Details</b>
	///
	...
	/// 
	/// \return As this is a <i>constructor</i> for the SomeClass class, nothing is returned
	///
	...
	///
	/// \see ~SomeClass(), ValidateDM1(), ValidateDM2() and ValidateDM3()
	///
	SomeClass(char* newDM1, int newDM2, int newDM3) : dm3(ValidateDM3(newDM3))
	{
	...
	/* NOTE : that in the sample comment below, I used the DOxygen @param directive - only to
			  show you that it still works in the C#-style commenting - you really wouldn't document the 
			  VOID parameter */

	///
	/// \brief <b>Brief Description</b> - Called upon to <i>destroy</i> a SomeClass object - once it loses <b>scope</b>
	/// \details <b>Details</b>
	///
	/// Here is where I can provide a better, more detailed description of what this method does...
	///
	...
	///
	/// \return As this is a <i>destructor</i> for the SomeClass class, nothing is returned
	///
	/// \see SomeClass()
	///
	~SomeClass()
	{
		printf("Dead SomeClass ...\n");
	}

	void SomeClassDump(void);
};
